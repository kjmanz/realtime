...
test('更新後もスマホIDで同じ席と予想済み状態に戻る', async () => {
  const host = await post('/api/rooms', { name: '固定進行', deviceId: 'device-host-001' });
  const code = host.code;
  const guest1 = await post('/api/rooms/join', { code, name: '固定一', deviceId: 'device-guest-001' });
  const guest2 = await post('/api/rooms/join', { code, name: '固定二', deviceId: 'device-guest-002' });
  const room = rooms.get(code);
  ...
  await action(code, host, 'start_vote');
  const suspectedId = room.round.minorityIds[0];
  for (const player of [host, guest1, guest2]) {
    await action(code, player, 'submit_vote', { ids: [suspectedId] });
  }

  const resumedGuest = await post('/api/rooms/join', { code, name: '固定一改', deviceId: 'device-guest-001' });
  assert.equal(resumedGuest.resumed, true);
  assert.equal(resumedGuest.playerId, guest1.playerId);
  assert.equal(room.players.length, 3);
  assert.equal(room.players.find((player) => player.id === guest1.playerId).submitted, true);
  assert.equal(room.players.find((player) => player.id === guest1.playerId).name, '固定一改');

  const resumedHost = await post('/api/rooms/join', { code, name: '固定進行', deviceId: 'device-host-001' });
  assert.equal(room.players.filter((player) => player.submitted).length, 3);
  await action(code, resumedHost, 'reveal_results');
  assert.equal(room.phase, 'results');
});
